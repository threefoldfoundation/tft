...
contract TFT is OwnedUpgradeableTokenStorage {
    ...
    function setSigners(address[] calldata newSigners,uint signaturesRequired) external onlyOwner {
        require(signaturesRequired > 0, "signaturesRequired must be greater than 0");
        require(newSigners.length > 0, "newSigners must be greater than 0");
        require(signaturesRequired <= newSigners.length, "signaturesRequired must be less or equal than the number of signers");
       
        deleteAddresses(keccak256(abi.encode("signers")));
